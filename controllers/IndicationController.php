...
class IndicationController extends Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['power', 'counter', 'membership', 'target', 'get-counter-start', 'get-membership-start'],
                        'roles' => ['writer'],
                    ],
                ],
            ],
    ...
    public function actionMembership($action, $id = null)
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($action === 'enable') {
                $model = new DataMembershipHandler(['scenario' => DataMembershipHandler::SCENARIO_ENABLE]);
                $model->load(Yii::$app->request->post());
                return $model->enable();
            }
        }
     ...
     * @throws Throwable
     */
    public function actionTarget($action, $id = null)
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        if (Yii::$app->request->isPost) {
            if ($action === 'enable') {
                $model = new DataTargetHandler(['scenario' => DataTargetHandler::SCENARIO_ENABLE]);
                $model->load(Yii::$app->request->post());
                return $model->enable();
            }
        }
