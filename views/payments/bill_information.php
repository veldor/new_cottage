        ...
        if(!empty($bill_info->billTarget)){
           ...
           foreach ($bill_info->billTarget as $item) {
               ...
               $payed = PayedTargetHandler::find()->where(['period_id' => $item->year_id])->all();
                ...
                try {
                    $payUp = TimeHandler::timestampToDate($targetInfo->pay_up_date);
                } catch (Exception $e) {
                    echo $e;
                    die;
                }
...
<div id="toolbar" class="no-print">
    <div class="btn-group pull-left">
        <a target="_blank" href="/print/bill/<?=$bill_info->bill->id;?>" class="btn btn-default"><span class="glyphicon glyphicon-print"></span> Распечатать квитанцию</a>
        <button type="button" class="btn btn-default activator" data-action="/send/bill/<?=$bill_info->bill->id;?>"><span class="glyphicon glyphicon-envelope"></span> Отправить квитанцию на почту</button>
        <button type="button" class="btn btn-default" id="printPageActivator"><span class="glyphicon glyphicon-print"></span> Распечатать детализацию</button>
        <a target="_blank" href="/pay/bill/<?=$bill_info->bill->id;?>" class="btn btn-danger"><span class="glyphicon glyphicon-usd"></span> Оплата</a>
    </div>
</div>
